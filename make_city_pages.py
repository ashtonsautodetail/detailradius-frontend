...
<style>
  :root {{ --navy:#0B1533; --navy2:#13276a; --cyan:#3B82F6; --ink:#1e293b; --muted:#556074; }}
  * {{ box-sizing:border-box; margin:0; padding:0; }}
  body {{ font-family:'Segoe UI',system-ui,-apple-system,sans-serif; color:var(--ink); background:#f6f9ff; }}
  a {{ color:inherit; }}
  .hero {{ background:linear-gradient(150deg,#0B1533 20%,#13276a 70%,#1e3a8a); color:#fff; padding:56px 20px 64px; text-align:center; }}
  ...
  .cta {{ display:inline-block; background:linear-gradient(135deg,#2563EB,#1D4ED8); color:#fff; font-weight:800; font-size:17px; padding:15px 34px; border-radius:12px; text-decoration:none; box-shadow:0 14px 30px -8px rgba(29,78,216,.5); }}
...
<section class="wrap" style="padding-top:0;">
  ...
  <div class="faq">
    ...
    <details><summary>I'm a detailer in {city} — can I join?</summary><p>Yes — that's the whole point. Free to join, no monthly fee, you keep 90% of every job, and deposits kill no-shows. <a href="/#detailers">Become a detailer</a>.</p></details>
  </div>
</section>
